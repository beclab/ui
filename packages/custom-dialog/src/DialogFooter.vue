<template>
    <div v-if="platform === 'mobile'" class="card-action row justify-between items-center">
        <slot name="footerMore" />
        <q-btn class="but-mobile-cancel" v-if="cancel" no-caps flat :style="{ width: cancel ? '48%' : '100%' }"
            @click="onCancel">
    ...
    <div v-else class="row justify-end items-center" style="padding: 0">
        <q-item v-if="skip" clickable dense class="but-skip row justify-center items-center" @click="onSkip">
            {{ skip === true ? 'Skip' : skip }}
        </q-item>
        ...
        <div class="row justify-center items-center">
            ...
            <q-item v-if="ok && !loading" clickable dense class="but-create row justify-center items-center"
                :class="[okDisabled ? 'ok-disabled' : '', okButtonClass]" @click="onSubmit" :style="okStyle">
                {{ ok === true ? 'Confirm' : ok }}
            </q-item>
        </div>
    </div>
